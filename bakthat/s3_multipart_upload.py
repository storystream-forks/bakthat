...
def multipart_upload(bucket, file_path, s3_key, file_size_in_mb, reduced_redundancy=False, acl='private'):
    cores = multiprocessing.cpu_count()
    if cores > 1:
        cores /= 2

    _multipart_upload(bucket, s3_key, file_path, file_size_in_mb, reduced_redundancy, cores)
    s3_key = bucket.get_key(s3_key)
